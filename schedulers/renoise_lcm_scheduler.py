...
class MyLCMScheduler(LCMScheduler):
    ...
    def step(
        ...
        if prev_step_index < len(sigmas):
            prev_sigma = sigmas[prev_step_index]
        else:
            prev_sigma = sigma

        # 2. compute alphas, betas
        alpha_prod_t = get_alphacumprod(sigma)
        alpha_prod_t_prev = get_alphacumprod(prev_sigma) #if prev_sigma >= 0 else self.final_alpha_cumprod
